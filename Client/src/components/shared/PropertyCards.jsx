...
const PropertyCards = ({ propertyData, searchString }) => {
  ...
  return (
    propertyData[searchString]?.map(prop => (
      <div key={prop._id} className="relative flex max-w-full flex-col overflow-hidden rounded-xl bg-white bg-clip-border text-gray-700 shadow-md">
      {/* Image */}
      ...
      <div className="p-4">
        ...
        <p className="mt-1">
          <span className="font-bold">From : </span>{`${prop?.price} AED`}
        </p>
        <p className="mt-1">
          <span className="font-bold">Developer : </span>{prop?.developer}
        </p>
      </div>
